...
def update_combined_epg():
    # --- إعدادات القناة الخاصة بـ Playco ---
    # المعرف الذي طلبته: 720335400128
    playco_ch_id = "720335400128"
    
    # توليد الوقت التلقائي (من بداية اليوم الحالي ولمدة 24 ساعة)
    start_ts = int((datetime.now(timezone.utc)).replace(hour=0, minute=0, second=0).timestamp())
    end_ts = start_ts + 86400 

    # الرابط المحدث للقناة المطلوبة فقط
    ...
    # --- إعدادات شاهد و m3u4u (كما كانت في سكريبتك) ---
    ...
    root = ET.Element("tv", {"generator-info-name": "Custom Combined EPG"})
    ...
    # --- 2. جلب بيانات Playco (القناة المحددة فقط) ---
    try:
        print(f"📡 جاري جلب بيانات القناة المحددة من Playco: {playco_ch_id}")
        res = requests.get(playco_url, headers=headers)
        ...
        for event in events:
            # تحويل Unix Timestamp إلى تنسيق XMLTV
        ...
        print(f"⚠️ خطأ في جلب بيانات Playco: {e}")

    # --- 3. جلب ودمج بيانات m3u4u ---
    try:
    ...
    print("✅ تم التحديث بنجاح! الملف جاهز باسم combined_final.xml")
